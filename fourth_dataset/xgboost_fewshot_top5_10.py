...
class ExperimentConfig:
    ...
    # Feature subsets mapped to the Cardiovascular Disease schema (Top 5 & Top 10)
    TOP_5_FEATURES = [
...
for config_name, feature_list in ExperimentConfig.FEATURE_SETS.items():
    ...
    output_dir = f"cardio_xgboost_baselines_{config_name}"
    os.makedirs(output_dir, exist_ok=True)

    # Inner loops: Few-shot constraints and iterations
    for k in ExperimentConfig.K_VALUES:
        res_file = os.path.join(output_dir, f'cardio_xgboost_results_granular_k{k}.csv')
