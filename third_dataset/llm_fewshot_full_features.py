...
class ExperimentConfig:
    ...
    # RECTIFIED: Exactly 25 real features present in the cleaned data schema
    FEATURES = [
...
def create_medical_prompt(support_df, test_batch):
    ...
    for _, row in support_df.iterrows():
        ...
        for f in ExperimentConfig.FEATURES:
            val = row[f]
            # Handle mixed variable formatting cleanly (Integers vs Floats/Strings)
...
def run_experiment():
    ...
    for k in ExperimentConfig.K_VALUES:
        results_file = f'heart_attack_results_granular_k{k}.csv'
        start_iter = 0
...
if __name__ == "__main__":
    try:
        run_experiment()
        print("\n" + "="*50 + "\n[SUCCESS] Heart attack dataset execution pipeline finalized.\n" + "="*50)
