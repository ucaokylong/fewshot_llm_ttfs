...
class ExperimentConfig:
    ...
    ITERATIONS = 20
    BATCH_SIZE = 20  # Điều chỉnh dựa trên VRAM (A40 46GB có thể lên 40-50)
        ...
        if os.path.exists(results_file):
            try:
                ...
                if valid_iters:
                    ...
                    print(f"[#] K={k} | Resume từ Iteration {start_iter}")
            except: pass

        for i in range(start_iter, ExperimentConfig.ITERATIONS):
            print(f"\n[EXEC] Qwen | K={k} | Iteration {i+1}/{ExperimentConfig.ITERATIONS}")
            ...
            # Lưu sau mỗi Iteration để đảm bảo an toàn dữ liệu
