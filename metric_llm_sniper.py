...
FOLDERS_CONFIG = {
    "llm_sniper_top5": "qwen_sniper_top5_summary.csv",
    "llm_sniper_top10": "qwen_sniper_top10_summary.csv"
}
...
def calculate_sniper_metrics():
    ...
    for folder_name, summary_output in FOLDERS_CONFIG.items():
        ...
        print(f"\n{'='*65}")
        print(f"📊 ĐANG TÍNH METRIC CHO THƯ MỤC: {folder_name.upper()}")
        print(f"{'='*65}")
        ...
        for k in K_VALUES:
            ...
            file_path = os.path.join(folder_name, f'qwen_sniper_k{k}.csv')
            ...
            try:
                ...
                print(f"[ERROR] Lỗi đọc file K={k} tại {file_path}: {e}")
                continue
        ...
        if summary_data:
            ...
            print(f"\n[SUCCESS] Đã lưu bảng tổng hợp vào: {summary_output}")
