...
class ExperimentConfig:
    ...
    # Path mappings for the newly balanced Framingham files
    ...
    ITERATIONS = 20
    BATCH_SIZE = 20  # Optimized based on VRAM capacity
...
def create_medical_prompt(support_df, test_batch):
    ...
    for _, row in support_df.iterrows():
        # Cleanly map values: handle integer conversion for nominal indicators safely
        feat_strs = []
...
def extract_predictions(raw_output, test_ids):
    ...
    # Method 1: Absolute JSON parsing approach
    try:
        ...
        # Method 2: Fallback regular expression parsing to preserve partial successes
        for tid in test_ids:
            pattern = rf"{tid}\D*?(\d)"
...
def run_experiment():
    ...
    for k in ExperimentConfig.K_VALUES:
        results_file = f'framingham_results_granular_k{k}.csv'
        start_iter = 0
...
if __name__ == "__main__":
    try:
        run_experiment()
        print("\n" + "="*50 + "\n[SUCCESS] Cross-dataset evaluation sequence finalized.\n" + "="*50)
